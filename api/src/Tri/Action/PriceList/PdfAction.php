...
final class PdfAction
{
    public function __construct(
        private readonly PriceListPdfRenderer $renderer,
        private readonly PriceListRepository $repo,
    ) {}
    ...
    public function __invoke(Request $request, Response $response, array $args): Response
    {
        ...
        $id = (int) ($args['id'] ?? 0);
        $list = $this->repo->find($id, $supplierId);
        if ($list === null) {
        ...
        try {
            ...
            ob_end_clean();
            $code = str_contains($e->getMessage(), 'položky') ? 'empty' : 'pdf_failed';
            $status = $code === 'empty' ? 400 : 500;
        ...
        $filename = $this->renderer->filename($list);
