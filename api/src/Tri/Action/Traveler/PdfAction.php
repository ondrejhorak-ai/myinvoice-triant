...
final class PdfAction
{
    public function __construct(
        ...
        private readonly TravelerRepository $repo,
    ) {}
    ...
    public function __invoke(Request $request, Response $response, array $args): Response
    {
        ...
        $id = (int) ($args['id'] ?? 0);
        $traveler = $this->repo->find($id, $supplierId);
        if ($traveler === null) {
        ...
        try {
            ...
            ob_end_clean();
            $status = str_contains($e->getMessage(), 'nenalezena') ? 404 : 500;
            $code = $status === 404 ? 'not_found' : 'pdf_failed';

            return Json::error($response, $code, $e->getMessage(), $status);
        } catch (\Throwable $e) {
            ob_end_clean();
        ...
        return self::pdfResponse($response, $path, TravelerPdfRenderer::filenameForTraveler($traveler), $download);
    }

    public static function pdfResponse(Response $response, string $path, string $filename, bool $download): Response
    {
